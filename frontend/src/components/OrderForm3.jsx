...
function PlaceOrderPage() {
    ...
    const fetchBpAddresses = async (cardCode) => {
      try {
        const addrRes = await API.get(
          `/sap/business-partners/${encodeURIComponent(cardCode)}/addresses`
        );
        ...
        if (addrRes.data?.status === "success") {
          ...
          const defShip =
            shipList.find((x) => x.IsDefault) || shipList[0] || null;
          const defBill =
            billList.find((x) => x.IsDefault) || billList[0] || null;
  ...
  useEffect(() => {
    const s = bpAddresses.shipTo.find(
      (x) => x.AddressName === order.shippingAddress
    );
    setShipToFull(formatAddressForDisplay(s, "ship"));
  }, [order.shippingAddress, bpAddresses.shipTo]);

  useEffect(() => {
    const b = bpAddresses.billTo.find(
      (x) => x.AddressName === order.billingAddress
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 🔹 Send request to Laravel backend via shared API instance
      const res = await API.post("/sap/sales-orders", payload);
    ...
    <div className="max-w-full mx-auto order-form-page w-full py-2 px-6">
      ...
      <div className="grid grid-cols-1 grid-cols-2 w-full gap-5 lg:gap-26 py-3">
        <form
          ...
          className="grid lg:grid-cols-2 grid-cols-1 gap-y-3 gap-x-6"
        >
          ...
          <div className="grid grid-cols-[max-content_1fr] items-center gap-2 w-full">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const a = bpAddresses.shipTo.find(
                  (x) => x.AddressName === val
                );
          ...
          <div className="grid grid-cols-[max-content_1fr] items-center gap-2 w-full">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const a = bpAddresses.billTo.find(
                  (x) => x.AddressName === val
                );
            ...
            <table className="table-auto w-full border-collapse px-2 rounded-xl">
              <thead className="sticky top-0 z-20 bg-white/50 backdrop-blur-sm shadow-[0_4px_6px_rgba(0,51,255,0.2)] font-semibold">
                <tr className="text-xs font-semibold align-middle border">
                  <th className="px-2 py-2 text-left w-2/12 font-semibold border-r border-[#0033ff]">
                    Item No.
                  </th>
                  <th className="px-2 py-2 text-left w-2/12 font-semibold border-r border-[#0033ff]">
                    Item Description
                  </th>
                  <th className="px-2 py-2 text-left w-1/12 font-semibold border-r border-[#0033ff]">
                    Quantity
                  </th>
                  <th className="px-2 py-2 text-left w-1/12 font-semibold border-r border-[#0033ff]">
                    Unit Price
                  ...
                  <th className="px-2 py-2 w-1/12 font-semibold border-[#0033ff]">
                    Delete
                  </th>
                </tr>
              </thead>
                        ...
                        <div className="relative">
                          ...
                          {items.length > 0 && (
                            <ul
                              ...
                              className={`bg-white absolute z-10 left-full ml-2 w-56 border rounded shadow-md w-full text-xs mt-1 max-h-40 overflow-y-auto ${
                                dropdownDir[index] === "up"
                                  ? "bottom-0 mb-1"
                                  : "top-0 mt-1"
                              }`}
                            >
